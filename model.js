...
/ Task 4/Scenario: remove specified permission type(s) from a given user and file; but the permission is actually inherited from the folder

//Function takes 3 parameters and returns nothing. Once the frontend is debugged
// this will be toggled with the original advanced/perm checkbox functionality and
// the UI will enable users to remove an inherited permission from a specified file given the user and the mapped permissions 
// Panel: (deny/allow) --> is a good key word to find the connection points in helper, view, and baseline

function overRideInheritedPermissions(file, user, permsEditable) {
  //Checks for "always" true condition for parent perms
    if (file.using_permission_inheritance) {
      //sets this to false to prevent reflect altered state
        file.using_permission_inheritance = false;

        //calls helper to identify inheritance tree
        let parentPerms = getACLHelper(file);

        file.acl = parentPerms.map(entry => ({ ...entry }));
    }
    //we call this here because once we identify everything we want our changes to exit any recursive steps
    removePermsHelper(file, user, permsEditable)
}
//identifies inheritance tree from a given file
function getACLHelper(file) {
    return file.parent ? file.parent.acl : [];
}
//removes the perms being edited 
function removePermsHelper(file, user, permsEditable) {
    file.acl = file.acl.filter(function(entry) {
        return !(entry.who === user && permsEditable.includes(entry.permission));
    })
}

//Psuedo Code for UI Integration --> Confused where to put it atm 
...
overRideInheritedPermissions(file, username, permsEditable);
*/
